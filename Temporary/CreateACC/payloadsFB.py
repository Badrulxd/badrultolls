        ...
        with requests.Session() as r:
            try:
                ...
                data.update({
                    ...
                    'doc_id':'8726416154089321',
                })
                response3 = r.post('https://m.facebook.com/api/graphql/', data=data, headers={'Accept':'*/*','Accept-Encoding':'gzip, deflate','Accept-Language':'en-US,en;q=0.9','Content-Type':'application/x-www-form-urlencoded','Origin':'https://www.facebook.com','Sec-Ch-Prefers-Color-Scheme':'dark','Sec-Ch-Ua':'','Sec-Ch-Ua-Full-Version-List':'','Sec-Ch-Ua-Mobile':'?0','Sec-Ch-Ua-Model':'','Sec-Ch-Ua-Platform':'','Sec-Ch-Ua-Platform-Version':'','Sec-Fetch-Dest':'empty','Sec-Fetch-Mode':'cors','Sec-Fetch-Site':'same-origin','User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36'}, cookies={"cookie":cookies}, allow_redirects=True).text
                if '"single_edit_image"' in response3 or '"settings_dropdown_profile_picture"' in response3: return (True)
                else: return (False)
            except (Exception) as e: pass
            
    def ProfileChek(self, cookies):
        if self.UploadPP(cookies): return("[green]Success edit profile")
        else: return("[red]Invalid edit profile")
...
class Require:
  def __init__(self):
    ...
    self.pw = ("Zoraa"+str(random.randint(11,29)))
...
class Main:
  ...
  def timer(self, seconds):
    while seconds:
        ...
        timeformat = ' [b green]![white]) time delay perdetik: [green]{:02d}:{:02d}'.format(mins, secs)
  ...
  def Next_Konfirmasi2(self, uid):
    ...
    header = {
      ...
      'sec-fetch-user': '?1',
      'upgrade-insecure-requests': '1',
      'user-agent': f'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{random.randint(108,128)}.0.0.0 Mobile Safari/537.36',
      'viewport-width': '980',      
    }
    params = {
    ...
    if "home.php?confirmed_account" in str(response.text):
      ...
      RequLink().FLCookies(cookies); RequLink().ReactPost(cookies)
      editP = LinkProf().ProfileChek(cookies)
      Console().print(f"""[b white]{{
 [white]status: [green]success
 [white]recovery code: [green]{kode}
 [white]details[white]: {{
   [white]username: [green]{self.firstnama} {self.lastnama}
   [white]password: [green]{self.pw}
   [white]userID: [green]{uid}
   [white]ultah: [green]{self.tgl}
   [white]email: [green]{self.mail}
   [white]profiles: [green]{editP}
   [white]cookies: [green]{cookies}[white]
 }}
}}""")
      with open("/sdcard/OK/createFB_success.txt","a") as wr:
        wr.write(f"{uid}|{self.firstnama} {self.lastnama}|{self.pw}|{self.tgl}|{self.mail}|{cookies}\n")
        wr.close()
    else:
      cookies = '; '.join([f'{key}={value}' for key, value in self.xyz.cookies.get_dict().items()])
      self.firstnama, self.lastnama, self.pw, self.tgl = self.instance.ResponseSM()  
      Console().print(f"""[b white]{{
 [white]status: [yellow]chekpoint
 [white]recovery code: [yellow]{kode}
 [white]details[white]: {{
   [white]username: [yellow]{self.firstnama} {self.lastnama}
   [white]password: [yellow]{self.pw}
   [white]userID: [yellow]{uid}
   [white]ultah: [yellow]{self.tgl}
   [white]email: [yellow]{self.mail}
   [white]cookies: [yellow]{cookies}[white]
 }}
}}""")
      with open("/sdcard/CP/createFB_invalid.txt","a") as wr:
        wr.write(f"{uid}|{self.firstnama} {self.lastnama}|{self.pw}|{self.tgl}|{self.mail}|{cookies}\n")
        wr.close()
